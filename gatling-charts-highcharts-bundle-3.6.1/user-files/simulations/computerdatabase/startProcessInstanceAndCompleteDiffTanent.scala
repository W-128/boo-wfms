...
class startProcessInstanceAndCompleteDiffTanent extends Simulation {
  ...
  object CompleteTask {
    ...
    val tanentAcomplete = tryMax(100){
      exec(
        http("completeTask-tanentA-rtl0")
          .post("/activiti-process-execution/completeTask/${tanentA-processDefinitionId}/${tanentA-processInstanceId}/${tanentA-taskId}?rtl=0")
          .check(status.is(200))
      )
    }.exitHereIfFailed
    val tanentBcomplete = tryMax(100){
      exec(
        http("completeTask-tanentB-rtl5")
          .post("/activiti-process-execution/completeTask/${tanentB-processDefinitionId}/${tanentB-processInstanceId}/${tanentB-taskId}?rtl=5")
          .check(status.is(200))
      )
    }.exitHereIfFailed
  }
